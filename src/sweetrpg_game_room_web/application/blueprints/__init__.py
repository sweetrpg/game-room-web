...
def main_page():
    ...
    if user_id:
        ...
        context.update({
            ...
            'wishlist_recent': [
                {**w, 'updated_at_label': _format_date(w.get('updated_at') or w.get('created_at'))}
                for w in (_recent_entries(wishlists, 'updated_at')
                          or _recent_entries(wishlists, 'created_at')
                          or wishlists[:3])
            ],
